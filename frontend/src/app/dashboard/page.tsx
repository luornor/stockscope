...
export default function DashboardPage() {
  ...
  const [section, setSection] = useState<"overview" | "news" | "watchlist" | "charts">("overview");

  const defaultByMarket: Record<Market, string> = { international: "AAPL", ghana: "MTNGH" };
  const [symbol, setSymbol] = useState<string>(defaultByMarket[market]);

  useEffect(() => { setSymbol(defaultByMarket[market]); }, [market]);

  const { quotes } = useQuotes(market);
  const picked = useMemo(
    () => quotes.find((q) => q.symbol === symbol) || { symbol, name: symbol, price: 0, change: 0, volume: 0 },
    [quotes, symbol]
  );

  const { series, isLoading: iLoading } = useIntraday(symbol, market);
  const { movers } = useMovers(market, 6);
  const { news } = useNews(market, market === "international" ? symbol : undefined);
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 md:grid md:grid-cols-[18rem_1fr] md:gap-6">
        ...
        <main className="mt-6 md:mt-10 w-full">
          ...
          {section === "overview" && (
            <>
              {/* Top controls */}
              <div key={`top-${market}`} className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <div className="text-xl font-semibold text-slate-100">
                        {picked.symbol}{" "}
                        <span className="text-slate-400 text-sm font-normal">• {picked.name}</span>
                      </div>
                    </div>
                    <WatchlistButton symbol={picked.symbol} name={picked.name} market={market} />
                  </div>
                  <ChartPanel symbol={picked.symbol} data={series} />
                  {iLoading && <div className="mt-2 text-xs text-slate-500">Loading chart…</div>}
                </div>

                <div>
                  <SymbolPicker market={market} value={symbol} onChange={setSymbol} />
                </div>
              </div>
              ...
              {/* Secondary Row */}
              <div key={`lower-${market}`} className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-white/10 p-4 bg-white/5">
                    ...
                    <div className="mt-2 text-xs text-slate-500">Live data auto-refreshes.</div>
                  </div>
                </div>
          ...
          {section === "watchlist" && (
            <div className="mt-6">
              {quotes.length ? (
                ...
                <div className="rounded-2xl border border-white/10 p-4 bg-white/5 text-sm text-slate-400">
                  Nothing in watchlist yet. Pick a symbol and press “Watch”.
                </div>
              )}
            </div>
          )}
            ...
            <>
              ...
              <div className="mt-6 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xl font-semibold text-slate-100">
                    {market === "international" ? "International" : "Ghana"} • {symbol}
                  </div>
                </div>
              ...
              <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-white/5 p-3">
                  ...
                  {iLoading && (
                    <div className="mt-2 text-xs text-slate-500">Loading chart…</div>
                  )}
                  {market === "ghana" && (
                    <div className="mt-3 text-[11px] text-slate-500">
                      Ghana intraday currently shows a same-day line (live vs prev close).
                    </div>
                  )}
                </div>
              ...
              <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  {quotes.length ? (
                    ...
                    <div className="text-xs text-slate-500">No quotes available for this market.</div>
                  )}
                </div>
                <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-sm text-slate-300 mb-2">Related News</div>
                  {news.length ? (
                    <NewsList
                      items={news.map((n) => ({
                        id: n.id,
                    ...
                    <div className="text-xs text-slate-500">No recent headlines for this market.</div>
                  )}
                </div>
              </div>
            </>
          ...
          <div className="py-8 text-center text-xs text-slate-500">Live backend • © Stock-Scope</div>
        </main>
      </div>
    </div>
  );
